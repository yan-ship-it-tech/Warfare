...
# Pass 27 — OSM query prep: buildings, residential roads, and a real rail-selector gap

No fetch, no new `data/osm/*.json`. This is `scripts/fetch-osm-data.mjs` schema
work done while drafting Overpass QL for four *urban* AOIs (ordinary
residential/mixed-use districts in Mykolaiv, Kyiv, Rostov-on-Don and Moscow),
requested for a future OSM inset outside Pokrovsk/Kramatorsk's rural
rail-and-tree-row scope.

## 1. Buildings were never fetched

`FEATURE_CLASSES` had no `building` selector at all — a residential-district
export under the existing four classes (rail/tree_row+forest+wood/road/river)
comes back with the road grid and nothing else, which is what the user
noticed and asked about. Added as its own key, deliberately **not** in
`DEFAULT_FEATURES`: a rural rail/tree-row AOI has no use for building
footprints and they would dwarf the rest of the file, so a run targeting a
dense district needs `--features=rail,trees,roads,rivers,buildings`
explicitly. No `classify()` gap once the selector exists — it needed one new
`if (tags.building) return "building"` line, same pattern as every other
type. `closed` needed no new logic either: `reduce()` already derives it
generically from first-point/last-point equality, and most building ways are
closed rings.

## 2. `roads` excluded `highway=residential`

The original selector was `~"^(primary|secondary|tertiary)$"` — fine for a
rural AOI where those are most of the road network, but a residential
district's street grid is almost entirely `highway=residential`, which that
regex silently excludes. Widened to
`~"^(primary|secondary|tertiary|residential)$"`. `classify()` needed no
change here either — `if (tags.highway) return "road"` already accepts any
value; this was purely a query-selector gap, same shape as the buildings gap
above.

## 3. Found while verifying, not asked for: the rail selector was never actually widened

Cross-checking the regenerated queries against Pass 15's documented fix (this
doc, above) turned up a real latent gap: Pass 15 widened `RAIL_TRACK_VALUES`
in `classify()` to accept `disused`/`abandoned`/`construction`/
`narrow_gauge`, which is why Pokrovsk's rail layer went from 1 feature to
303 — but `FEATURE_CLASSES.rail`'s query *selector* was never touched and
still read `way["railway"="rail"]`. Every `--print-query` since Pass 15,
across every AOI, has been asking Overpass for active rail only;
Pokrovsk's 303-feature layer exists solely because a human ran a second,
separate `way["railway"]` query by hand outside the script and merged the
result in manually (Pass 15 §2, "Merge, not replace"). The fix `classify()`
shipped was real and necessary but incomplete — it fixed what happens to
data once fetched, not what gets asked for. Widened the selector to bare
`way["railway"]` (any value) so a live or `--print-query` run finds
disused/abandoned track in one pass, matching what `classify()` has accepted
since Pass 15.

## What this pass did not do

No fetch — egress is still blocked in this sandbox (see "Status" in
`docs/OSM_PIPELINE.md`); the four district queries were handed to the user
as `--print-query` output plus overpass-turbo.eu pre-filled links
(`?Q=<url-encoded query>`) for them to run and export by hand, the same
GeoJSON-paste route Pass 14 established. No `AOIS` entries were added for
the four districts (Mykolaiv/Namyv, Kyiv/Troieshchyna, Rostov/Severny,
Moscow/Konkovo) — queries were generated by borrowing an existing AOI id
with `--bbox=` override rather than committing placeholder entries whose
`name`/`note` would say the wrong place. Real `AOIS` entries land once an
export actually comes back and is being reduced for real, not before.

## Where the brief and prior passes disagreed

Nothing above changes `DEFAULT_FEATURES`, so `--print-query`/`--aoi=pokrovsk`
and `--aoi=kramatorsk` with no `--features=` flag are unaffected for
buildings/residential (opt-in only, as designed) — but the rail-selector
widening **does** change their default output: a bare re-run of either
stored AOI will now ask Overpass for more rail than before Pass 27. That's a
deliberate correction of a gap Pass 15 left standing, not a new design
choice, and is called out here rather than landing silently.

## Verification

`node --check scripts/fetch-osm-data.mjs` clean. Ran
`--aoi=kramatorsk --bbox=<each of the four district boxes>
--features=rail,trees,roads,rivers,buildings --print-query` for all four and
inspected the emitted QL by eye: bare `way["railway"]`, the widened highway
regex, and `way["building"]` all present, bbox substituted correctly, no stray
AOI name/note leakage into the query text (the query only ever contains the
bbox, not the borrowed AOI's identity). No `npm run build` needed — no `src/`
or `data/` file touched, only the fetch script.
