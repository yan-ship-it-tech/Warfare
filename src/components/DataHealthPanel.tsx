...
import type { DataIssue, WorldModel } from "../data/model";
...
export function DataHealthPanel({ world }: { world: WorldModel }) {
  ...
  if (view.openPanel !== "health") return null;

  const shown = SEVERITY_ORDER.filter((s) => filter === "all" || filter === s);

  return (
    <div className="modal" role="dialog" aria-modal="true" aria-label="Data health">
                    ...
                    <div className="health__meta">
                      ...
                      {issue.subject && (
                        <button
                          ...
                          onClick={() => {
                            const target =
                              world.assetsById.get(issue.subject!) ??
                              world.stubsById.get(issue.subject!);
                            if (target) {
